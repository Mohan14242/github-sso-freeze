...
export default function ServicesList() {
  ...
  async function load() {
    try {
      setLoading(true)
      // Backend now returns only the services this user's teams can see.
      // No frontend filtering by role needed — just render what comes back.
  ...
  // Client-side search filter (within already-scoped results)
  ...
  return (
    <div style={{ padding: "40px 48px", maxWidth: 900 }}>
      ...
      {/* Search — only show when there are enough services to warrant it */}
      ...
      {totalCount === 0 && (
        <div style={{
          ...
          <p style={{ color: "#334155", fontSize: 14, margin: 0 }}>
            No services found. You may not have access to any services yet.
          </p>
        </div>
      )}

      {/* Pending approval cards (blurred overlay) */}
      {pending.map(req => (
